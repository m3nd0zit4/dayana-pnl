...
- **Free webinar**: `get_free_webinar`, `update_free_webinar`, `deactivate_free_webinar`. Single public landing at `/webinar-gratuito` (also linked from `/enlaces` when live). Schedule date+time are always entered in the CRM timezone `America/Bogota` — never as free text and never in the visitor's timezone. Publishing (`isActive: true`) requires headline, subheadline, date+time, and at least one learn item. Use the `free-webinar` skill for the guided flow. Registrations are tagged `webinar-gratuito`.
