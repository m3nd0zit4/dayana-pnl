...
const CheckoutPaymentModalFrame = ({
  ...
  return (
    <div
      ...
      data-lenis-prevent
      className="fixed inset-0 z-[101] flex items-end justify-center bg-[#141118]/55 p-0 backdrop-blur-md animate-[checkoutModalFadeIn_200ms_ease-out] sm:items-center sm:p-4 lg:p-6"
      onMouseDown={onBackdropClick}
    >
      <div
        data-lenis-prevent
        style={modalStyle}
        className="checkout-payment-modal relative flex w-full max-w-lg flex-col overflow-x-hidden overflow-y-auto overscroll-y-contain rounded-t-2xl border border-black/10 bg-white text-[#141118] shadow-[0_40px_120px_-20px_rgba(0,0,0,0.35)] animate-[checkoutModalPopIn_240ms_cubic-bezier(0.22,1,0.36,1)] sm:rounded-2xl"
      >
        <div
          aria-hidden="true"
        ...
        <div className="checkout-payment-modal__header sticky top-0 z-20 flex shrink-0 items-center justify-between gap-3 border-b border-black/10 bg-white/95 px-5 pb-4 pt-[max(1.25rem,env(safe-area-inset-top))] backdrop-blur-md sm:px-6">
          {header}
          <button
            type="button"
            ref={closeButtonRef}
            onClick={onClose}
            aria-label="Cerrar"
            className="flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-full text-black/45 transition-colors hover:bg-black/[0.06] hover:text-black"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
