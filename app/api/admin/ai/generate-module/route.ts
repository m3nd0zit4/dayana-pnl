...
import { z } from "zod";
import { generateModuleContent } from "@/lib/ai/module-generation";
...
export async function POST(req: NextRequest) {
  ...
  const rl = await rateLimitDistributed(`ai-generate:${staff.id}`, 10, 60_000);
  if (!rl.ok) {
  ...
  try {
    const content = await generateModuleContent(parsed.data.prompt);
